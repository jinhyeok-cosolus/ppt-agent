...
-   공통 Header, Logo, 제목 위치, 기본 폰트 패밀리, 페이지 번호 등은
    Hard Rule을 따른다.
-   이 레이아웃의 상단 Summary Bar(§2, 필수 요소)는 이미 "제목을
    보완하는 한 줄 설명"의 역할을 겸하므로, Hard Rule §12 Main Title
    Supporting Message(Optional 부제목)는 Summary Bar와 별도로 추가
    적용하지 않는다 — 제목 아래에 Summary Bar와 Supporting Message를
    이중으로 배치하지 않는다.
...
-   Fill: Hard Rule §10 Header Variant Background 값(`#F4FAFA`)과
    동일한 값을 참조한다 — 이 문서가 별도 값을 새로 정의하지 않는다.
-   Border / Rule: Hard Rule §5 Brand Color의 **Primary** 토큰을
    참조한다.
...
-   Bar는 좌측 Table 영역에만 걸치지 않고 **좌·우 콘텐츠 전체를 하나로
    묶는 전폭 요소**로 사용한다.
-   Bar Fill은 반드시 Hard Rule §10 Header Variant Background 값을
    사용한다.
-   Bar 경계선은 반드시 Hard Rule §5 Primary 토큰을 사용한다.
...
Divider의 두께·색상·기본 스타일은 이 문서에서 재정의하지 않으며,
**Hard Rule §11 Vertical Content Divider** 스펙을 그대로 따른다.
...
Section Title의 기본 색상은 Hard Rule §5 Brand Color의 **Dark/Main**
토큰을 사용한다(이 문서는 별도 리터럴 값을 정의하지 않는다).
...
Process Diagram의 색상은 특정 색상 계열을 고정 팔레트로 지정하지 않고,
Hard Rule §5 Brand Color 및 Claude PPT Design System §2 Color System의
Brand Color Palette(Main Color 계열의 Teal 톤 + Ink/Gray)를 따른다.

Neutral: - Dark: `#263238` - Gray: `#78909C` - Light Gray: `#B0BEC5`,
`#CFD8DC`

Red/Blue 등 Main Color 계열 외 색상은 Hard Rule §5에 따라 명확한 의미가
필요한 예외적인 경우에만 제한적으로 사용한다. 시각적 다양성 확보만을
목적으로 한 임의의 색상 추가는 금지한다.
...
### 반드시 유지할 색상
  ...
  Element                     Color
  --------------------------- -----------------------
  Summary Bar Fill            Hard Rule §10 Header Variant Background 값
  Summary Bar Border / Rule   Hard Rule §5 Primary 토큰
  Section Title               Hard Rule §5 Dark/Main 토큰
  Main Teal Diagram Element   Hard Rule §5 Primary 토큰
  Dark Process Panel          `#263238`(§6 Diagram Color Palette 전용 Neutral, Header/Divider와 무관)

데이터 강조·상태 표시(Primary Data Element, Highlight, Positive/Active
Status 등)에 쓰이는 색상은 고정 Hex를 지정하지 않고, Hard Rule §5 /
Claude PPT Design System §2의 Brand Color Palette(Main Color 계열)에서
선택한다.

AI가 임의로 새로운 메인 컬러 팔레트를 생성하지 않는다.

콘텐츠 특성상 추가 색상이 필요한 경우에도 Hard Rule §5 Brand Color
Palette를 우선 사용한다.
...
2.  Hard Rule §10 Header Variant Background / §5 Primary / §5 Dark-Main
    토큰 기반의 색상 일치(리터럴 값은 §2/§4/§7 참조)
