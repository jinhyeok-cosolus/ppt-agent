...
## 새 Field Test 사례 추가 방법

1. 새 프로젝트가 [6] Human Review ②를 통과해 확정되면(`state.json.stage`가 6 이후로 진행하고 사용자가 실제로 승인한 버전), 그 확정 결과가 이 문서의 기존 Pattern과 같은 콘텐츠 구조/분량 조건에 해당하는지 확인한다.
2. **같은 Pattern에 해당** → 해당 섹션의 "Field Test 근거"에 프로젝트명과 신뢰도(§ "신뢰도 표기 기준")를 추가한다. 기존 서술과 다른 세부 변형이 있으면 하위 항목(예: "2-A"/"2-B"처럼)으로 분리해 기술한다. 중간에 시도했다가 되돌린 구조는 성공 사례로 추가하지 말고, 원인이 명확하면 Avoid When에만 추가한다.
3. **새로운 Pattern** → 위 각 섹션과 동일한 스키마(Content Relationship / Content Density / 항목 수 / Visual / 강조 정보 / 최종 확정 Layout·Variant / Use When / Avoid When / Field Test 근거)로 새 섹션을 추가한다.
4. 특정 회사명·제품명·문구는 기록하지 않는다 — 콘텐츠 구조와 분량 조건만 일반화해서 남긴다.
5. 이 문서를 수정한 뒤에는 별도 승인 절차 없이 즉시 반영해도 된다(Layout MD/Hard Rule 갱신([9] 단계)과 달리, 이 문서는 사용자의 실제 확정 이력을 그대로 옮기는 기록 문서이므로 CLAUDE.md 2장 [9]의 "사용자 명시 승인 필요" 규칙 대상이 아니다). 다만 이 문서 자체의 서술이 Layout MD의 구현 규칙과 충돌하면 Layout MD가 우선한다.
