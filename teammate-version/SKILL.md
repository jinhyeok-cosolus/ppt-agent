...
- `scripts/bundle_for_share.py` — 특정 `vN/` 버전을 외부 공유용 단일 self-contained HTML(`shared.html`)로 번들링(CSS·이미지·로컬 스크립트 인라인). 메인 에이전트가 "공유 링크 게시" 절차에서 직접 실행하며, LLM 판단이 필요 없는 순수 파일 변환이다. content-designer가 직접 호출할 일은 없다.
