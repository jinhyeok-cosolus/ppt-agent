...
본문은 Hard Rule §9의 본문 Safe Area(**X 64~1216px, Y 135~656px, 즉
1152×521px Body Box**)를 100% 기준으로 아래 구조를 따른다. 실제 좌표는
`64 + (X% × 11.52)`, `135 + (Y% × 5.21)`로 환산한다.
...
| Vertical Divider (§11) | D% 지점(= Header Gap의 정중앙과 동일 좌표) | Benefit Area Body(Content Area) 내부, 상하 약 8%씩 inset한 구간(기본값, Content Area의 약 84% 커버, 세로 중심은 Content Area 중심과 일치) | Header 영역(Header Gap 포함) 비침범, Footnote가 있으면 그 상단까지의 Content Area 기준([5](#5-vertical-divider-hard-rule-11-적용) 참조) |
...
하나의 통합 Header처럼 보이지 않도록 분리하는 간격이다. 기본값은 Body
Box 폭 기준 **약 1.5%**(1152px 기준 약 17px)이며, 콘텐츠·Header 문구
길이에 따라 1~2% 범위에서 조정할 수 있다. Gap은 두 Header가 서로
...
-   본 Layout의 좌/우 Benefit은 Before/After처럼 기존 대비 개선을
    ...
    Header는 Hard Rule §10이 정의하는 두 스타일(Fill 없는 스타일 /
    `#F4FAFA` Fill 스타일) 중 **하나로 통일**해 양쪽에 동일하게
    적용한다. 한쪽에만 다른 스타일을 부여해 두 Benefit 사이에 우열이
    있는 것처럼 보이게 하지 않는다.
...
-   Divider는 Content Area의 세로 중앙 범위를 중심으로 배치하며, 좌우
    ...
    커버한다. **기본값**은 Content Area 상·하단에서 각각 Content Area
    높이의 약 8%씩 inset한 구간(전체의 약 84%를 커버)이며, Divider의
    세로 중심은 Content Area의 세로 중심과 일치시킨다. 이 8%/84%
    수치는 Reference pptx 실측값이 아니라 "좌우 Benefit이 충분히
    구분되면서도 자연스러워 보인다"는 정성적 요구를 구체화한
    기본값이므로, 콘텐츠 구성에 따라 조정할 수 있다.
...
-   좌우 영역 비율(D, 50~55% 범위)
-   분할 Header 사용 시 Header Gap(G, 1~2% 범위)
-   Header 구성 — 통합 1개 / 분할 2개 중 콘텐츠 구조에 따라
    [4. Header 판단 기준](#header-판단-기준)을 적용해 결정
-   분할 Header 사용 시, Header(§10) 스타일 중 어느 쪽 변형을 좌우
    공통으로 사용할지(Fill 없음 / `#F4FAFA` Fill 중 선택, 단 좌우
    동일 적용)
