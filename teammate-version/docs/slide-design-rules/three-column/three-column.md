...
각 Column 상단에는 **Hard Rule §10 Content Comparison Header** 스타일을
그대로 적용한 Header Bar를 사용한다. Height·Font·Font Color·Letter
Spacing·정렬·Padding·Corner·Fill·Divider 등 Header Bar 자체의 시각
스펙은 이 문서에서 재정의하지 않으며, Hard Rule §10 원 스펙을 그대로
따른다. 이 문서(Layout MD)가 결정하는 것은 Header의 **개수(3개)·폭·
Content Group 대응 관계**, 그리고 Hard Rule §10이 정의하는 두 Fill
변형 중 어느 것을 쓸지뿐이다.

-   3개 Header Bar는 **Hard Rule §10**(2개 이상의 독립 Header Bar
    사용 시 Gap 확보 의무)에 따라 서로 맞닿지 않고 시각적으로 구분되는
    **Gap을 두고 배치**한다 — Header Bar끼리 이어 붙어 하나의 긴
    Header처럼 보이지 않아야 한다. Gap의 구체적인 폭은 Column 폭·
    콘텐츠에 맞게 이 Layout이 결정한다. 각 Header Bar 내부에는 해당
    Column의 핵심 주제를 짧고 명확하게 표기한다.
-   3개 Header Bar의 **X/Y 위치, 폭(Gap 제외), 높이를 모두 동일하게
    유지**한다.
-   3개 Column은 Before/After처럼 대립하는 두 대상이 아니라 대등하고
    독립적인 병렬 항목이므로, Hard Rule §10의 두 Fill 변형(Fill 없는
    스타일 / `#F4FAFA` Fill 스타일) 중 **하나를 선택해 3개 Header Bar
    모두에 동일하게 적용**한다 — Column마다 다른 변형을 섞어 쓰지
    않는다.
-   **기본값은 `#F4FAFA` Fill + Primary Divider 변형**이다. Fill 없는
    스타일은 Before/Existing처럼 대립하는 두 대상을 구분할 때 사용하는
    변형이므로, 대등한 3개 병렬 Column의 기본값으로 사용하지 않는다.
...
-   Divider는 Header Bar 바로 아래에 붙여서 시작하지 않는다. Header
    ...
    영역(Key Message~Supporting Evidence, §5~§6 참조) **내부에서
    상·하단을 inset**해 배치한다 — Divider는 Header가 아니라 본문
    Content Group을 구분하는 요소로 인식되어야 한다. Inset 폭은
    Column Content 영역 높이의 약 10~15%를 기본으로 하되, Key
    Message 등 상단 요소와 겹치지 않는 범위에서 콘텐츠 구성에 맞게
    조정할 수 있다.
