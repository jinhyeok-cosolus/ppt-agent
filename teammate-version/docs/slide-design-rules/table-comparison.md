...
-   **Table이 슬라이드의 핵심 Visual인 경우**(다른 Chart/Image 없이
    ...
    기본값대로 Body Box(1152×521px) 전체 폭을 넓게 사용한다.
...
기준값이 Body Box(1152×521px)가 아니라 Table에 할당된 Content Area로
바뀐다.

본문은 Hard Rule §9의 본문 Safe Area(**X 64~1216px, Y 135~656px,
1152×521px Body Box**)를 100% 기준으로 아래 구조를 따른다.
...
-   **직각형 Table 구조를 유지한다.** Rounded Corner, Card 그림자,
    부유형(Floating) Cell을 사용하지 않는다 — 모든 Cell은 인접 Cell과
    완전히 맞닿아 하나의 격자를 이룬다.
-   Cell 구분은 **얇은 직선(1px 내외)** 으로만 표현한다. 굵은 테두리,
    이중선, 강한 그림자로 Cell을 강조하지 않는다.
-   불필요한 Cell Fill과 장식을 최소화한다 — 기본 상태의 Body Cell은
    무Fill(투명) 또는 아주 옅은 중립색만 사용하고, 강조는
    [9. 자사 강조](#9-자사-강조)에서 정의한 범위 안에서만 적용한다.
...
-   Cell 내부 여백(Padding)은 텍스트/이미지가 Cell 경계에 바짝 붙지
    않을 정도로 상하좌우 균등하게 확보한다 — Reference의 Padding
    수치는 PowerPoint 자동맞춤(spAutoFit)으로 생성된 값이라 그대로
    복제하지 않고, Design System의 일반 여백 기준을 따른다.
...
-   **Data Comparison Table 기본형**은 비교 대상 개수와 무관하게
    Header Row 전체를 **Main Color Fill + White Bold Text**로
    통일한다(Claude PPT Design System §8 Table Style 기본값). Header
    Row를 Column/대상 단위로 나눠 일부만 Fill하거나 다른 스타일을
    섞어 쓰지 않는다 — 여러 선택지를 두지 않고 이 기본값 하나로
    통일한다.
-   **Reference가 있는 경우**, 위 기본값보다 Reference가 실제로 사용한
    Header 강조 방식을 우선 적용한다.
-   콘텐츠 분량에 따라 Row 높이 자체가 달라지는 것은
    [4. Row 구조](#4-grid-column--row-규칙)를 따른 것이며 위 색상
    통일 규칙과는 별개다.
-   본 Layout은 **Hard Rule §10 Content Comparison Header를
    사용하지 않는다** — §10은 Before/After·Benefit Area처럼 정확히
    2개 영역을 대비시키는 2-Column 전용 Header 스펙이며, N개
    Column(가변)로 구성되는 Table Header에는 적용 범위가 다르다.
    Header Row의 색상·Typography는 [10. Typography](#10-typography)와
    Visual Style(Design System)을 따른다.
...
자사(COSOLUS) 또는 강조하려는 대상의 Column/Row는 아래 범위 안에서만
강조한다 — 기존 Hard Rule/Visual Style을 그대로 적용하고 이 문서에서
새 강조 스타일을 만들지 않는다.

-   **COSOLUS 강조는 Body Cell Fill이 아니라 Main Color Text +
    Bold로 표현한다.** 기존/비교 대상의 Body Text는 Black/Dark를
    유지한다.
-   **COSOLUS 강조를 Row 또는 Column 전체 배경 Fill로 고정하지
    않는다** — [비교축 선택 원칙](#비교축-선택-원칙)에 따라 COSOLUS가
    Row에 배치되든 Column에 배치되든 이 원칙은 동일하게 적용된다.
    [6. Header Row](#6-header-row)의 Header Row 전체 Fill은 대상
    식별과 무관한 일반 Header 스타일이며, COSOLUS만 별도로 Fill하는
    예외는 두지 않는다.
-   Border 강조(Cosolus Teal, 다른 Column/Row보다 굵은 두께)와
    Typography Weight 상향(SemiBold~Bold)을 함께 사용해 텍스트 강조를
    보강할 수 있다.
-   불가피하게 옅은 Fill을 사용하는 경우에도 Hard Rule §5 Color
    Usage가 정의한 Main Color 계열의 저채도 Tint만 사용하고, 다른
    Column과 명도 대비가 과하지 않게 한다.
-   **Reference가 있는 경우**, 위 기본 강조 방식보다 Reference가
    실제로 사용한 강조 방식을 우선 적용한다.
-   Orange/Amber 계열은 Hard Rule §5에 따라 핵심 수치·제목 강조에
    사용하지 않는다.
-   강조는 최대 1개 Column(또는 1개 Row)까지만 적용한다 — 2개 이상을
    동시에 강조하면 우선순위가 흐려진다.
...
Design System의 기존 Typography Tier 체계(다른 Layout Reference와
동일 척도)를 그대로 사용한다. 이 범위를 벗어난 임의의 Font Size/Weight를
새로 만들지 않는다.
...
| Header Row Label / Criteria Column Label | 14pt | SemiBold |
| Body/Data Cell(Variant A 수치·짧은 텍스트) | 16pt | Regular~Light |
| Cell 설명 캡션(Variant B 이미지 하단) | 14pt | Light(자사 강조 Cell만 SemiBold) |
| Footnote / Highlight Band | 12pt | Light |
...
-   Hard Rule §10 Content Comparison Header를 이 Layout의 Header
    Row에 그대로 적용
