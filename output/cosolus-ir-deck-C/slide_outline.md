# COSOLUS IR Deck (C) — Slide Outline
...
- 본 outline은 `slide-content-structuring` Skill을 정상 호출해 작성됨(콘텐츠 역할·관계·Region·Layout Routing·구조적 사전 점검을 이 단계에서 판단, web-ppt-generator는 이 판단을 그대로 소비).
...
- **Layout Selection Reason**: 표지 슬라이드는 Hard Rule에 따라 항상 이 문서를 우선 적용. 배경 이미지는 `docs/brand-assets/cover-images/` 중 화학소재·공정기술을 동시에 다루는 본 발표 주제에 맞는 후보를 web-ppt-generator 단계에서 최종 선택.
...
- **Structural Check**: 우측(산업적 요인) 정보량이 좌측(환경적 요인 4개 수치)보다 현저히 적음(NC-01, 시계열 데이터 없음) — 균형을 맞추기 위해 임의로 막대그래프를 만들지 않고, 우측은 CAGR 31.6%를 Large Number로 강조 + 배경 설명 문장으로 시각적 밀도를 보완(웹PPT 단계에서 좌우 Visual Weight 조정, 가짜 수치로 채우지 않음).
...
- **Structural Check**: 각 Column의 Main Visual은 원문에 세부 수치·이미지가 없으므로 Icon+Message 방식으로 구성 예정(콘텐츠에 없는 수치를 임의로 만들지 않음). **[C 특기사항]** three-column.md §5의 "Visual 영역 55~65% 적극적 채움" 요구를 web-ppt-generator 단계 QA에서 반드시 대조할 것 — 아이콘 단독 배치로 Visual 컨테이너 대비 실제 채움 밀도가 낮아지지 않도록 아이콘 크기/배경 처리/보조 요소 구성을 Layout Compliance Check에서 확인한다.
...
- **Structural Check**: 문제 없음. 하단 4개 한계는 독립적으로 이해 가능하도록 구분하고 개선 방향(소재·공정 개선 필요)으로 수렴시킨다.
...
- **Layout Selection Reason**: 비교 대상이 기존/개선 정확히 2개이며, 두 화학소재(이미지) 자체의 직접 비교가 핵심 메시지 — Variant A의 Main Visual 유형을 "Image ↔ Image"(화학구조식)로 적용.
- **Structural Check**: 문제 없음. 원문에 구체적 수치가 없어 "재활용 효율 개선"이라는 정성적 결론만 사용하고 임의 수치를 추가하지 않는다.
...
- **Structural Check**: table_7의 공정시간 행은 원본 셀이 그래픽으로 처리되어 있어 인접 문단 텍스트로 재구성한 값임(NC 아님, material_analysis.json B08 기록 참고, 수치 자체는 원문 그대로: 벨기에 100%↑/중국 50%↑). "기존 추출제"(B09, 5단/3.6톤)는 3사 비교 표와 별도 축(세대 비교)이므로 같은 표에 억지로 4번째 열로 합치지 않고 Highlight Band 문장으로 분리해 정보 왜곡을 방지. **[C 특기사항]** table-comparison.md §6/Claude_PPT_Design_System.md §8의 "Header Row 전체 통일 Fill" 요구, §5의 "Cell 중앙 정렬 기본" 요구, 표 하단 빈 공간 방지(`height:100%` 등)를 web-ppt-generator 단계 QA에서 반드시 대조할 것.
...
- **Selected Layout**: `before-after/before-after.md` — **Variant A (Process Transformation)**
- **Layout Selection Reason**: "1단계(단순 증발) vs 4단계(다단 정제 추출)"라는 공정 구조 자체의 차이가 메시지 핵심 — Variant A 정확히 부합. Main Visual은 Process/Step Sequence.
...
- **Layout Selection Reason**: 비교 대상 4개, 동일 기준 반복 비교, 텍스트 밀도가 높아 표 형태가 Diagram보다 정확 — Table Comparison Use When에 정확히 부합. Multi-Radar Do Not Use 조건("정성 설명·항목별 주석이 많아 정확한 수치를 그대로 읽어야 할 때 → Table 유지")에 해당.
- **Structural Check**: 4행 모두 텍스트 분량이 있어 Row 높이를 콘텐츠 기준으로 배분(균등 고정 아님). COSOLUS 자사 강조는 이 슬라이드에서는 특정 열이 자사가 아니므로(범주 비교) 적용하지 않음.
...
- **Structural Check**: `*리튬선물 가격 약 44,100천원/ton 기준`은 Footnote로 작게 표기. **[C 특기사항]** benefit-impact.md §1의 "Main Title에서 이미 전달된 Core Technology를 본문에서 재요약 금지" 원칙과 §7 Avoid의 "큰 KPI 숫자 + 넓은 여백" 금지 패턴을 web-ppt-generator 단계 QA에서 반드시 대조할 것 — 상단 Core Technology 요약이 Main Title과 중복되지 않는지, Evidence 영역이 숫자만 덩그러니 있는 넓은 여백 패턴이 되지 않는지 확인.
...
- **Selected Layout**: `before-after/before-after.md` — **Variant A (Process Transformation)**
- **Layout Selection Reason**: 두 공정의 단계 구성 자체(유도가열/부유선별/건식환원 경로 vs COSOLUS 통합 경로)가 핵심 메시지 — Variant A 부합. 실사진(img62~65)은 §4.9 Result/Output Visual로 COSOLUS측에 보완 배치.
- **Structural Check**: 문제 없음. 원문에 정량 비교 수치가 없어(정성적 "경제성·친환경성·양산성 확보"만 존재) 임의 수치를 추가하지 않는다 — 정량 비교는 다음 슬라이드(솔루션2 경쟁력)에서 다룬다.
...
- **Structural Check**: 문제 없음. 상단 Process Visual에 실제 COSOLUS 설비 사진을 우선 사용(실제 자사 설비 사진이 있으므로 일반 아이콘보다 우선).
...
- **Layout Selection Reason**: "기존 대비 무엇이 얼마나 개선되는가"를 다수의 동일 기준으로 보여주는 것이 핵심 — Variant B에 정확히 부합. 비교 대상이 정확히 2개뿐이므로 Comparison Matrix/Table Comparison보다 Before-After Variant B가 적합.
- **Structural Check**: 비교 기준이 6~7개로 Variant B 권장 범위(3~6개) 상한에 가까움 — 우선순위가 낮은 기준은 상위 기준에 통합해 5~6개로 조정 검토(웹PPT 단계 반영 필요). `019_competitive-advantage-highlight.md`가 기술하는 "COSOLUS vs BTR vs Vianode" 3사 비교는 본 프로젝트 원본 자료에 해당 경쟁사명·구체 수치가 confirm되지 않으므로 그대로 가져오지 않는다 — 원본에 있는 "기존 소성로(Pusher Kiln)" 대비 비교만 사용. **[C 특기사항]** before-after.md §5.1의 기본 Column 폭 비율(existing 39% : improved 41%, Improved가 더 넓어야 함)이 실제 CSS에 정확히 반영되는지 web-ppt-generator 단계 QA에서 반드시 대조할 것.
...
- **Layout Selection Reason**: Executive Summary/overview/key takeaways 목적에 정확히 부합하는 카탈로그 항목. 좌우 2개의 대등한 섹션(기술·사업 요약 vs 투자 정보)으로 자연스럽게 나뉜다.
...
- **Structural Check**: NC-04에 따라 일본 기업 로고(Panasonic Energy/Iwatani/DNP)는 원문의 "일본 현지투자사·재료업체 등"이라는 익명 표현과 특정 매칭이 확인되지 않으므로 **사용하지 않는다** — 인도네시아 측은 원문 맥락과 정합적인 로고(SWAP/MUKTI/eCoNiL/IBC/HLI)와 실사진만 사용하고, 일본 측은 텍스트 설명만으로 구성해 확정되지 않은 파트너십을 암시하지 않는다.
...
## A와의 슬라이드 매핑 (참고용, C 테스트 보고 목적)

C의 [3]은 A와 동일한 material_analysis.json·동일 skill·동일 조건으로 독립 수행한 결과이며, 슬라이드 번호·제목·Selected Layout이 A와 1:1로 동일하게 대응된다. 즉 A의 Slide 4(Three-Column)↔C Slide 4, A Slide 7(Table Comparison)↔C Slide 7, A Slide 10(Benefit+Impact)↔C Slide 10, A Slide 13(Before-After Variant B)↔C Slide 13. 위 4개 슬라이드에는 A에서 발견된 QA 결함 항목을 "[C 특기사항]"으로 Structural Check에 명시해, web-ppt-generator 단계의 QA 절차가 이를 재현 여부까지 대조하도록 표시해 두었다.
